...
BEGIN
  ...
  VALUES (auth.uid(), 'delete_shop', 'seller', p_user_id, json_build_object('deleted_products', v_count, 'forced', p_force)::jsonb);

  RETURN json_build_object('success', true, 'deleted_products', v_count);
END;
$$;

REVOKE ALL ON FUNCTION public.admin_delete_shop(uuid, boolean) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.admin_delete_shop(uuid, boolean) TO authenticated;